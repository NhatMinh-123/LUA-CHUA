...
std::vector<int> strokesPlayer1;
std::vector<int> strokesPlayer2;
...
void loadLevel(int level)
{
	if (level > 4)
	{
		strokesPlayer1.push_back(balls[0].getStrokes());
		strokesPlayer2.push_back(balls[1].getStrokes());
		state = 2;
		return;
	}
...
void graphics()
{
	...
	else
	{
		...
		int y_offset = 96;
		for (size_t i = 0; i < strokesPlayer1.size(); ++i) {
			std::string text = "Level " + std::to_string(i + 1) + ": Player 1: " + std::to_string(strokesPlayer1[i]) + " strokes, Player 2: " + std::to_string(strokesPlayer2[i]) + " strokes";
			window.renderCenter(0, y_offset, text.c_str(), font24, white);
			y_offset += 32;
		}
	}
	window.display();
